...
*if you don't want to go through the trouble of putting quotations around each word in the sentence, you can use the str_split() function stringr:* 
  library(stringr)
...
*Here you wanted to extract every number between the number at row 2 column 6 and row 6 to column 9. Remember that comma after the number indicates rows, and before the number indicates columns. So pointing out a number at a row and column, you write [2,6].  By using the colons (i.e 2:6), you are pulling out everything between rows 2 to 6 because a colon means "through". But if you are trying to extract [2,6] through [6,9], you can write x<-m[2,6]:m[6,9]. -Mel*
